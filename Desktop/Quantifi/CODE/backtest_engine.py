...
class BacktestEngine:
    ...
    def run_backtest(self):
        ...
        while current_date < self.end_date:
            ...
            if liquidation_orders and self.params['liquidation']:
            ...
            if self.params['rebalancing']:
                self.portfolio_manager.process_rebalancing(current_date) #processed rebalancing is done before checking to avoid look ahead bias
            ...
            if self.strategy == 'momentum':
                ...
                if len(self.portfolio_manager.POSITIONS) > 0:
                    if self.params["regime_filter"] and not bull_market:
                ...
                if self.params["regime_filter"]:
                    if not bull_market:
                        self.signal_generator.mean_reversion_entry_signals_adf(current_date, prices, reversion_coins, self.portfolio_manager.POSITIONS, 
                ...
                if len(self.portfolio_manager.POSITIONS) > 0:
                    ...
                    if self.params["regime_filter"]:
                        if bull_market:
                            ...
                            self.signal_generator.mean_reversion_exit_signals(current_date, self.portfolio_manager.POSITIONS, signals_type='long')
                        else:
                            print('Exiting all long positions.')
                            self.portfolio_manager.exit_all_positions(position_type='long')
                            self.signal_generator.mean_reversion_exit_signals(current_date, self.portfolio_manager.POSITIONS, signals_type='short')
                    
                    else:
                        self.signal_generator.mean_reversion_exit_signals_adf(current_date, self.portfolio_manager.POSITIONS)
            ...
            btc_price = self.data_manager.fetch_current_price('BTC-USD', current_date, 'Close')
